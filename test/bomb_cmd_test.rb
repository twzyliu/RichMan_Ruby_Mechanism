...
class BombCmdTest < Test::Unit::TestCase
  ...
  def test_should_wait_for_cmd_after_bomb
    @player.set_statu Status::WAIT_FOR_CMD

    assert_equal Status::WAIT_FOR_CMD, (@player.command Commands::BOMB_CMD)
  end

  def test_should_change_toolnum_when_bomb
    @player.set_statu Status::WAIT_FOR_CMD
    @player.bomb.set_num TestHelper::ENOUGH_TOOLS
    tools_num = @player.tools_num

    @player.command Commands::BOMB_CMD.set_parameter(TestHelper::TOOL_STEP)

    assert_equal tools_num - 1, @player.tools_num
  end

  def test_should_change_land_status_when_bomb
    @player.set_statu Status::WAIT_FOR_CMD
    @player.bomb.set_num TestHelper::ENOUGH_TOOLS
  ...
  def test_should_not_change_when_target_land_too_faraway
    ...
    @player.command Commands::BOMB_CMD.set_parameter(TestHelper::LONG_STEP)
