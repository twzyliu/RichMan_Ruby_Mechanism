...
class Command_Test < Test::Unit::TestCase
  ...
  def test_should_return_wait_for_cmd_after_respondless_cmd
    flexmock(Commands, :execute => Status::WAIT_FOR_CMD)

    assert_equal Status::WAIT_FOR_CMD, (@player.command Commands)
  end

  def test_should_return_wait_for_response_after_respondful_cmd
    flexmock(Commands, :execute => Status::WAIT_FOR_BUY_RESPONSE)

    assert_equal Status::WAIT_FOR_BUY_RESPONSE, (@player.command Commands)
  end

  def test_should_return_turn_end_after_after_noneed_respond_cmd
    flexmock(Commands, :execute => Status::TURN_END)

    assert_equal Status::TURN_END, (@player.command Commands)
  end
